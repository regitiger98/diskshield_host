...
typedef struct ms_ecall_tPrepareForCertificateProvisioning_t {
	...
	uint64_t ms_nonce64;
	const sgx_target_info_t* ms_pTargetInfo;
...
typedef struct ms_ecall_tGenM7_t {
	ae_error_t ms_retval;
	const SIGMA_S1_MESSAGE* ms_pS1;
	const EPID11_SIG_RL* ms_pSigRL;
	uint32_t ms_nTotalLen_SigRL;
	const uint8_t* ms_pOcspResp;
	uint32_t ms_nTotalLen_OcspResp;
	const uint8_t* ms_pVerifierCert;
	uint32_t ms_nTotalLen_VerifierCert;
	const pairing_blob_t* ms_pPairingBlob;
...
typedef struct ms_ecall_tVerifyM8_t {
	ae_error_t ms_retval;
	const SIGMA_S3_MESSAGE* ms_pS3;
	uint32_t ms_nTotalLen_S3;
	const EPID11_PRIV_RL* ms_pPrivRL;
	uint32_t ms_nTotalLen_PrivRL;
	pairing_blob_t* ms_pPairingBlob;
...
sgx_status_t ecall_tGenM7(sgx_enclave_id_t eid, ae_error_t* retval, const SIGMA_S1_MESSAGE* pS1, const EPID11_SIG_RL* pSigRL, uint32_t nTotalLen_SigRL, const uint8_t* pOcspResp, uint32_t nTotalLen_OcspResp, const uint8_t* pVerifierCert, uint32_t nTotalLen_VerifierCert, const pairing_blob_t* pPairingBlob, uint32_t nMaxLen_S2, SIGMA_S2_MESSAGE* pS2, uint32_t* pnTotalLen_S2)
{
	sgx_status_t status;
	ms_ecall_tGenM7_t ms;
	ms.ms_pS1 = pS1;
	ms.ms_pSigRL = pSigRL;
	ms.ms_nTotalLen_SigRL = nTotalLen_SigRL;
	ms.ms_pOcspResp = pOcspResp;
	ms.ms_nTotalLen_OcspResp = nTotalLen_OcspResp;
	ms.ms_pVerifierCert = pVerifierCert;
...
sgx_status_t ecall_tVerifyM8(sgx_enclave_id_t eid, ae_error_t* retval, const SIGMA_S3_MESSAGE* pS3, uint32_t nTotalLen_S3, const EPID11_PRIV_RL* pPrivRL, uint32_t nTotalLen_PrivRL, pairing_blob_t* pPairingBlob, uint8_t* bNewPairing)
{
	sgx_status_t status;
	ms_ecall_tVerifyM8_t ms;
	ms.ms_pS3 = pS3;
	ms.ms_nTotalLen_S3 = nTotalLen_S3;
	ms.ms_pPrivRL = pPrivRL;
	ms.ms_nTotalLen_PrivRL = nTotalLen_PrivRL;
	ms.ms_pPairingBlob = pPairingBlob;
